...
# ===== 파일 경로(리포 루트) =====
PLAYERS_CSV   = "aram_participants_clean_preprocessed.csv"       # ✅ 새 전처리 본판
ITEM_SUM_CSV  = "item_summary_with_icons_from_clean.csv"         # ✅ 새 전처리 아이템 요약
...
def load_players(path: str) -> pd.DataFrame:
    ...
    # 아이템 이름 정리 (item0_name ~ item6_name 가정)
    for c in [c for c in df.columns if re.fullmatch(r"item[0-6]_name", c)]:
        df[c] = df[c].fillna("").astype(str).str.strip()

    # 스펠 표준 컬럼(전처리에서 만들어둔 컬럼 우선)
    for c in ["spell1_u","spell2_u","spell_pair","spell1","spell2","spell1_name_fix","spell2_name_fix"]:
        if c in df.columns:
            df[c] = df[c].fillna("").astype(str).str.strip()

    # 룬/챔피언
    for c in ["rune_core","rune_sub","champion"]:
        if c in df.columns:
            df[c] = df[c].fillna("").astype(str).str.strip()
    return df
...
def load_item_summary(path: str, df_for_fallback: pd.DataFrame|None) -> pd.DataFrame:
    """요약 파일이 없으면 참가자 데이터에서 즉석 생성(아이콘은 빈 값)."""
    if _exists(path):
        g = pd.read_csv(path)
    ...
    # 폴백: 참가자에서 요약 만들기(아이콘은 비움)
    if df_for_fallback is None:
        return pd.DataFrame(columns=["item","icon_url","total_picks","wins","win_rate"])
    item_cols = [c for c in df_for_fallback.columns if re.fullmatch(r"item[0-6]_name", c)]
    stacks = []
    for c in item_cols:
...
# 전처리 표준 컬럼 우선 사용: spell1_u, spell2_u 존재 시 그것으로 집계
...
@st.cache_data
def resolve_spell_icon(name: str, spell_map: dict) -> str:
...
if games and s1 and s2:
    ...
    sp["spell1_icon"] = sp[s1].apply(lambda x: resolve_spell_icon(x, spell_map))
    sp["spell2_icon"] = sp[s2].apply(lambda x: resolve_spell_icon(x, spell_map))
